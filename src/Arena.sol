...
contract Arena {
    ...
    event BattleCreated(
        uint256 indexed battleId, address indexed challenge, uint256 entryFee, uint8 maxAgents
    );
    ...
    function createBattle(
        ...
        Battle newBattle =
            new Battle(challenge, entryFee, maxAgents, block.timestamp + duration, msg.sender);
    ...
    function registerAgent(
        ...
        require(
            IAttackRegistry(attackRegistry).isUnderAttack(challenge), "Challenge not in attack mode"
        );

        address agentOwner = _getAgentOwner(agent);
        require(msg.sender == agentOwner || msg.sender == agent, "Not agent owner");
    ...
    function getCreatorBattleCount(
        address creator
    ...
    function _getAgentOwner(
        ...
        require(agentOwner != address(0), "Invalid agent");
        return agentOwner;
    }
}
