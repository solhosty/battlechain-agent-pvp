...
contract BettingCaller {
    function commitBet(
        address payable bettingAddress,
        uint256 battleId,
        bytes32 commitHash
    ) external {
...
contract SpectatorBettingTest is Test {
    ...
    function _commit(
        ...
        bytes32 commitHash = betting.getCommitHash(bettor, BATTLE_ID, agentIndex, amount, salt);
        vm.prank(bettor);
    ...
    function testClaimPayoutDustGoesToLastClaimer() public {
        ...
        vm.prank(spectator1);
        betting.claimPayout(BATTLE_ID);
        assertEq(spectator1.balance - firstBalance, 1.5 ether);

        vm.prank(spectator2);
        betting.claimPayout(BATTLE_ID);
        assertEq(spectator2.balance - secondBalance, 1.5 ether);
    }
    ...
    function testPlaceBetFromContractReverts() public {
        ...
        bytes32 commitHash = betting.getCommitHash(address(caller), BATTLE_ID, 0, 1 ether, salt);
        vm.warp(START_TIME - delay - 1);

        vm.expectRevert("EOA only");
        caller.commitBet(payable(address(betting)), BATTLE_ID, commitHash);
    }
}
